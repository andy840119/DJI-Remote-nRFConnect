 ...
 *
 * conn_id is the Zephyr connection index (bt_conn_index(), uint16_t).  The
 * field name and type are kept from the ESP-IDF/NimBLE version so that
 * callers using ble_get_conn_id() need zero changes; ble.c maps it back to
 * the owning `struct bt_conn *`.
 */
typedef struct {
    uint16_t conn_id;
    int camera_index;
