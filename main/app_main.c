 ...
 * 
 * Hardware: Pro Micro nRF52840 (SuperMini / nice!nano compatible)
 * Display: 320x240 ST7789V TFT LCD
...
/* picolibc only exposes M_PI for the BSD/GNU feature sets */
#ifndef M_PI
...
/* Board HAL -- the target board is selected by the Zephyr board target, so
 * there is no Kconfig board chooser to switch on here (the original had one
 * because two ESP32 boards shared the firmware). */
#include "promicro_nrf52840_hal.h"
#define HAL_INIT()                promicro_nrf52840_init()
#define HAL_LVGL_INIT()           promicro_nrf52840_lvgl_init()
#define HAL_BACKLIGHT_ON()        promicro_nrf52840_backlight_on()
#define HAL_BTN_A_PRESSED()       promicro_nrf52840_button_a_pressed()
#define HAL_BTN_B_PRESSED()       promicro_nrf52840_button_b_pressed()
#define HAL_BTN_C_PRESSED()       promicro_nrf52840_button_c_pressed()
...
LOG_MODULE_REGISTER(app_main, CONFIG_DJI_REMOTE_LOG_LEVEL);
