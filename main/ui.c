 ...
 * 
 * Hardware: Pro Micro nRF52840 (320x240 ST7789V TFT LCD)
...
LOG_MODULE_REGISTER(ui, CONFIG_DJI_REMOTE_LOG_LEVEL);
 ...
 */
/* The Pro Micro nRF52840 has no external button header, and its free pins are
 * already taken by the display, the GPS UART and buttons A/B/C. */
...
static esp_err_t init_gpio_system(void) {
    ...
    (void)ret;  /* only used when external buttons are enabled */
    ...
    */
     ...
     * (moved inside the external-button guard: with no external buttons there
     * are no GPIO interrupts to service) */
#if UI_ENABLE_EXTERNAL_BUTTONS
